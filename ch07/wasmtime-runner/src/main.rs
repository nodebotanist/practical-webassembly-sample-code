...
pub extern "C" fn roll_dice_log() {
    ...
    if args.len() >= 4 {
        ...
        let dice_results = result.get_dice_rolls();
        println!("Result of {:?}d{:?}+{:?}: {:?} total: {:?}", number_of_dice, die_max, modifier, dice_results, result);
        ...
        if let Err(e) = writeln!(roll_log_file, "Result of {:?}d{:?}+{:?}: {:?}", number_of_dice, die_max, modifier, dice_results) {
